...
public class RandomValue {
    public static String intCharacter = "1234567890";
    ...
    public static long getLong(int length){
        int len = intCharacter.length();
        int index = 0;//表征字符串中字符索引
        char ch;//获取字符
        StringBuilder sb = new StringBuilder();
        for(int i=0;i<length;i++){
            index = (int)(Math.random() * len);
            ch = intCharacter.charAt(index);
            sb.append(ch);
        }
     ...
     * 返回手机号码
     */
    private static String[] telFirst="139,138,137,136,135,134,159,158,157,150,151,152,188,130,131,132,156,155,133,153,189,180,177,176".split(",");
    private static String getTel() {
        int index=getNum(0,telFirst.length-1);
        String first=telFirst[index];
        String second = String.valueOf(getLong(8));
...
//        return first+second+third;
    ...
    private static String getChineseName() {
        ...
        String first=firstName.substring(index, index+1);//获取姓氏
        int sex=getNum(0,1);
        String str=boy;
